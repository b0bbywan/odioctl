...
func TestUpgradesPathFor(t *testing.T) {
	tests := []struct{ state, want string }{
		{"", SystemUpgradesPath},
		{SystemStatePath, SystemUpgradesPath},
		{"/tmp/dev/state.json", "/tmp/dev/upgrades.json"},
	}
	for _, tc := range tests {
		if got := UpgradesPathFor(tc.state); got != tc.want {
			t.Errorf("UpgradesPathFor(%q) = %q, want %q", tc.state, got, tc.want)
		}
	}
}
