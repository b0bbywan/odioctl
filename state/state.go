...
// system cache path for the system state, a sibling file for a dev/test
// tree. `check` writes it, `apply` and `web` read it — one rule, so they
// always meet on the same file.
