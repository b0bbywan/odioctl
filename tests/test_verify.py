...
class VerifyChecksTests(unittest.TestCase):
    def test_features_known_passes_for_subset(self):
        ...
        self.assertIsNone(verify._warn_features_unknown(s))

    def test_features_unknown_warns(self):
        s = make_state(features=["tidal", "bogus"])
        warning = verify._warn_features_unknown(s)
...
class RunVerifyTests(unittest.TestCase):
    ...
    def test_unknown_feature_warns_but_returns_0(self):
        with tempfile.TemporaryDirectory() as d:
            path = write_state(d, make_state(odios="2026.5.0", features=["tidal", "newthing"]))
            err = io.StringIO()
            with contextlib.redirect_stderr(err):
                self.assertEqual(verify.run_verify(path, None), 0)
        self.assertIn("warning", err.getvalue())
        self.assertIn("newthing", err.getvalue())
