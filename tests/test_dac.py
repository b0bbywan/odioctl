...
class ParseTests(unittest.TestCase):
    ...
    def test_overlay_outside_the_catalog_is_left_alone(self):
        ...
        for name in ("hifiberry-amp4", "vc4-kms-v3d"):
            with self.subTest(name=name):
                self.assertFalse(dac.is_audio_overlay(name))
                text = f"dtoverlay={name}\n"
                self.assertEqual(dac.parse(text).stray_lines, [])
                applied = dac.apply(text, dac.BY_ID["hifiberry-dac"])
                self.assertNotIn(dac.DISABLED_PREFIX, applied)
...
class ApplyTests(unittest.TestCase):
    ...
    def test_onboard_sets_audio_on_without_overlay(self):
        ...
        # only the reset, never an overlay of its own
        self.assertEqual([ln for ln in block.splitlines() if "dtoverlay" in ln], ["dtoverlay="])

    def test_audio_param_is_targeted_at_the_base_dtb(self):
        ...
        for dac_id in (dac.ONBOARD, "hifiberry-dacplus-std"):
            with self.subTest(dac_id=dac_id):
                out = dac.apply(FIXTURE + "dtoverlay=vc4-kms-v3d\n", dac.BY_ID[dac_id])
                lines = out.splitlines()
                self.assertLess(
                    lines.index("dtoverlay=vc4-kms-v3d"),
                    lines.index("dtoverlay="),
                )
                self.assertEqual(
                    lines.index("dtoverlay=") + 1,
                    next(i for i, ln in enumerate(lines) if ln.startswith("dtparam=audio=")),
                )

    def test_params_are_rendered(self):
        entry = dac.DacEntry("hifiberry-dacplus-std", "X", "slave")
