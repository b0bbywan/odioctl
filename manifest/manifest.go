...
// upgradesCache is the slice of upgrades.json this package reads back. A
// report without target_tag is not one `check` wrote: it counts as absent.
...
func Resolve(version, upgradesPath string) (*Manifest, error) {
	if c, err := readCache(upgradesPath); err == nil && c.TargetTag == version {
		var m Manifest
