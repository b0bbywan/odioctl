...
func TestResolveReturnsCachedManifestOnVersionMatch(t *testing.T) {
	...
	path := writeUpgrades(t,
		`{"latest": "2026.5.0", "manifest": {"odios": "2026.5.0", "roles": {"mpd": "2026.5.0"}}}`)
...
func TestResolveFallsBackToFetchOnVersionMismatch(t *testing.T) {
	path := writeUpgrades(t, `{"latest": "2026.4.0", "manifest": {"odios": "2026.4.0", "roles": {}}}`)
...
func TestResolveVersion(t *testing.T) {
	...
	tests := []struct{ explicit, path, want string }{
		...
		{"", writeUpgrades(t, `{"latest": "2026.6.0"}`), "2026.6.0"},
