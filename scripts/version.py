#!/usr/bin/env python3
"""Version helpers — ``src/odioctl/__init__.py`` is the source of truth.
...
INIT = Path(__file__).resolve().parent.parent / "src" / "odioctl" / "__init__.py"
...
def normalize_tag(tag: str) -> str:
    ...
    if not m:
        sys.exit(
            f"tag {tag!r} doesn't match the canonical form vX.Y.Z or vX.Y.Z-{{rc,beta,alpha}}.N"
        )
