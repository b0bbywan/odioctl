...
func TestDacSetTakesFlagsOnEitherSideOfTheID(t *testing.T) {
	cfg := writeConfig(t)
	// Flags first: what a human types. The sudoers lines and the web put
	// the ID first (covered above), both must reach the same place.
	rc, out, _ := run(t, "dac", "set", "--dry-run", "--config", cfg, "hifiberry-dacplus-std")
	if rc != 0 || !strings.Contains(out, dac.Begin) {
		t.Errorf("rc = %d, out = %q", rc, out)
	}
	for _, argv := range [][]string{
		{"dac", "set", "--config", cfg},                    // no ID
		{"dac", "set", "a", "b", "--config", cfg},          // two IDs
		{"dac", "unset", "hifiberry-dac", "--config", cfg}, // unset takes none
	} {
		if rc, _, err := run(t, argv...); rc != 2 || !strings.Contains(err, "usage:") {
			t.Errorf("%v: rc = %d, stderr = %q", argv, rc, err)
		}
	}
}
...
func TestDacMissingConfigReturns2(t *testing.T) {
	...
	if rc != 2 || !strings.Contains(err, "/nonexistent not found") {
		t.Errorf("rc = %d, stderr = %q", rc, err)
	}
}
