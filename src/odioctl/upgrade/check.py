...
from odioctl import components, fsutil, manifest, state
...
def _write_upgrades_report(report: UpgradeReport, output: str) -> None:
    if directory := os.path.dirname(output):
    ...
    fsutil.atomic_write_json(output, report, indent=2, sort_keys=False)
