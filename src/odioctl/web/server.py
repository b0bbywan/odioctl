...
# A route returns the banner text and, for an action, the modal to show with it.
Outcome = tuple[str, "ActionResult | None"]


def _action_components(svc: Services, form: dict[str, str], _host: str) -> Outcome:
    msg = svc.set_component(form.get("kind", ""), form.get("name", ""), form.get("enabled") == "1")
    return msg, None


def _action_component_action(svc: Services, form: dict[str, str], host: str) -> Outcome:
...
def _action_dac(svc: Services, form: dict[str, str], _host: str) -> Outcome:
    dac_id = form.get("id", "")
    if not dac_id:  # an empty select is not an unset — that is /dac/unset
        raise WebError("no DAC selected")
    return svc.set_dac(dac_id), None


# The POST routes; ACTION_PATHS is derived so routing and dispatch cannot drift.
ACTIONS: dict[str, Callable[[Services, dict[str, str], str], Outcome]] = {
    "/components": _action_components,
    "/components/action": _action_component_action,
    "/dac": _action_dac,
    "/dac/unset": lambda svc, _form, _host: (svc.set_dac(None), None),
    "/upgrade": lambda svc, _form, _host: (svc.start_upgrade(), None),
}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if self._reject_path(path, "POST"):
            return
        try:
            msg, result = ACTIONS[path](self.services, self._read_form(), self._host())
        except TokenError as e:
            self._send_status(HTTPStatus.FORBIDDEN, f"<p>{render.escape(e)}</p>")
            return
        except WebError as e:
            page = render.render_page(
                self.services, error=str(e), result=e.modal, host=self._host()
            )
            self._send_html(HTTPStatus.OK, page)
            return
        self._send_html(
            HTTPStatus.OK,
            render.render_page(self.services, message=msg, result=result, host=self._host()),
        )
