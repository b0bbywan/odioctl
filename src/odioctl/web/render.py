...
`render_page` only reads from `Services`. The modal is passed in by the POST
that produced it, so an action's output goes to that one response and the
next page load is clean.
"""
