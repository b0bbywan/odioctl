...
func TestCheckWritesNextToACustomState(t *testing.T) {
	// No --output: the report lands where `apply` and `web` will look for it.
	d := t.TempDir()
	statePath := writeState(t, d, makeState())
	swapFetch(t, fetchOf(man("2026.5.0", map[string]string{"mpd": "2026.5.0"})))
	var stdout bytes.Buffer
	if rc := RunCheck(&stdout, os.Stderr, CheckOptions{State: statePath}); rc == 2 {
		t.Fatalf("rc = %d\n%s", rc, stdout.String())
	}
	if ReadReport(filepath.Join(d, "upgrades.json")) == nil {
		t.Error("no report next to state.json")
	}
}

func TestRefreshFetchesWhenTheTagChanges(t *testing.T) {
	// A cache written for another release is never re-stamped with the new
	// tag: `apply` would then skip roles that release actually bumps.
	d := t.TempDir()
	st := makeState()
	st.Roles = map[string]string{"mpd": "2026.5.0"}
	st.Features = []string{"mympd"}
	...
	out := filepath.Join(d, "upgrades.json")
	swapFetch(t, fetchOf(man("2026.5.0", map[string]string{"mpd": "2026.5.0"})))
	Refresh(CheckOptions{State: statePath, Output: out})
	swapFetch(t, fetchDown)
	if r := Refresh(CheckOptions{State: statePath, Version: "pr-84", Output: out}); r != nil {
		t.Errorf("offline report = %+v, want nil", r)
	}
	if got := ReadReport(out); got == nil || got.TargetTag != "2026.5.0" {
		t.Fatalf("cache after failed fetch = %+v", got)
	}
	urls := swapFetch(t, fetchOf(prereleaseMan))
	r := Refresh(CheckOptions{State: statePath, Version: "pr-84", Output: out})
	if r == nil || r.TargetTag != "pr-84" || !reflect.DeepEqual(r.Manifest, prereleaseMan) {
		t.Fatalf("report = %+v", r)
	}
	if len(*urls) != 1 {
...
func TestReportWithoutATagIsNotOurs(t *testing.T) {
	...
	if r := ReadReport(out); r != nil {
		t.Errorf("report = %+v, want nil", r)
	}
}

func TestRefreshReusesCachedManifestWithoutFetching(t *testing.T) {
	d := t.TempDir()
	...
	// user removes mympd from features; a toggle is not a network check
	st.Features = []string{}
	writeState(t, d, st)
	urls := swapFetch(t, fetchDown)
	second := Refresh(opts)
	if len(*urls) != 0 {
		t.Errorf("fetched %v, want no network", *urls)
	}
