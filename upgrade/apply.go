...
// targetManifest is the snapshot `check` cached when it recorded this very
// tag, else a fetch. Nil degrades, never blocks: no RUN_X skips, install.sh
// runs every role.
func targetManifest(report *Report, version string) *manifest.Manifest {
	if report != nil && report.TargetTag == version {
		return &report.Manifest
	}
	url, err := manifest.ManifestURL(version)
	if err == nil {
...
func buildApplyEnv(w io.Writer, st state.State, version, targetUser string, man *manifest.Manifest,
	...
	if opts.Progress {
		...
		// The odio_progress callback finds odio-api's upgrade.sock under
		// XDG_RUNTIME_DIR and gives up under root without one. This runs as
		// root through sudo, whose env_keep only applies when the calling
		// process carries the odioctl group — a user manager started before
		// the membership landed does not — so name the target user's runtime
		// dir here instead of trusting the environment to have kept it.
